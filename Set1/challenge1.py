...
def to_base_64(hex_string):
    ...
    for i in xrange(0, len(hex_string), 6):
        hex_chars = hex_string[
            i:i + 6] if (i + 6) < len(hex_string) else hex_string[i:]
...
def hex_chars_to_base64(hex_chars):
    ...
    for i in xrange(0, len(binary_string), 6):
        binary_base64_char = binary_string[i:i + 6] if (i + 6) < len(
            binary_string) else binary_string[i:] + '0' * (6 - (len(binary_string) - i))
...
def binary_char_to_base64(binary_base64_char):
    ...
    if(int_from_binary >= 0 and int_from_binary <= 25):
        ...
        base_64_char = '/'
    if(base_64_char == ''):
        sys.stderr.write('Invalid binary number given! Input: ' + str(
            binary_base64_char) + ' Value: ' + str(int_from_binary) + ' Exiting...')
        exit(1)
    return base_64_char
